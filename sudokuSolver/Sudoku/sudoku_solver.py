...
def solve_with_comparison(board):
    ...
    # Progress series: (elapsed_ms, cells_filled) snapshots per solver, used
    # to render the "solve progress over time" line chart on the frontend.
    heuristic_progress = []
    backtrack_progress = []
    dlx_progress = []
    ...
    heuristic_result = solve_with_performance_tracking(board_heuristic, n=n, progress=heuristic_progress)
    # normalize result
    ...
    # Run backtracking solver — informational only. It has no move-ordering
    # heuristic, so on larger grids (e.g. 16x16's 256 cells) it will usually
    # hit its time_limit without finishing; the elapsed time is still real
    # and shown to the user, with backtrack_success indicating whether it
    # actually solved the puzzle in time.
    start_time = time.time()
    backtrack_success = solve_backtracking(board_backtrack, n=n, progress=backtrack_progress)
    backtrack_time = (time.time() - start_time) * 1000

    #-- DLX Solver --
    start_time  = time.time()
    dlx_result  = solve_dlx(board_dlx, n=n, progress=dlx_progress)  # returns solved board or None
    ...
    progress = {
        'heuristic': heuristic_progress,
        'backtrack': backtrack_progress,
        'dlx': dlx_progress,
    }

    return overall_success, final_solution, heuristic_time, backtrack_time, dlx_time, backtrack_success, progress